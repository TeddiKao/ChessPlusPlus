...
import VariantRenameDialog from "@/features/variants/variantListing/stores/variantRenameDialog";
...
function VariantSelectionDialog({ actions, isOpen, onOpenChange, handleVariantSelection, selectedVariantId }: VariantSelectionDialogProps) {
					...
					{selectedVariantId && variants[selectedVariantId] && (
						<DialogFooter>
							{Object.entries(actions).map(([variantId, action]) => (
								<Button
									key={variantId}
									onClick={() => action.execute(selectedVariantId)}
